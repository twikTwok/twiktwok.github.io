var manUpObject,
      ...
      if (manUpObject[validMetaValues[e].manifestName])
        if ("icons" == validMetaValues[e].manifestName)
          for (var a = manUpObject.icons, n = 0; n < a.length; n++)
            a[n].sizes == validMetaValues[e].imageSize &&
              (validMetaValues[e].content = a[n].src);
        else
          (validMetaValues[e].content =
            manUpObject[validMetaValues[e].manifestName]),
            "display" == validMetaValues[e].manifestName &&
              "standalone" == manUpObject.display &&
              (validMetaValues[e].content = "yes");
    ...
    for (var e = 0; e < validLinkValues.length; e++)
      if (manUpObject[validLinkValues[e].manifestName])
        if ("icons" == validLinkValues[e].manifestName)
          for (var a = manUpObject.icons, n = 0; n < a.length; n++)
            a[n].sizes == validLinkValues[e].imageSize &&
              (validLinkValues[e].content = a[n].src);
        else
          validLinkValues[e].content =
            manUpObject[validLinkValues[e].manifestName];
    return validLinkValues;
  },
  generateMetaArray = function () {
    for (
      var e = generateFullMetaData(),
  ...
  makeAjax = function (e) {
    ...
    window.XMLHttpRequest &&
      (/^https?:\/\//i.test(e) ? (fulURL = e) : window.location.hostname,
      ((a = new XMLHttpRequest()).onreadystatechange = function () {
        4 == a.readyState && 200 == a.status && generateObj(a.responseText);
      }),
      a.open("GET", e, !0),
      a.send());
  },
  collectManifestObj = function () {
    for (
      var e = document.getElementsByTagName("link"), a = 0;
      a < e.length;
      a++
    )
      e[a].rel && "manifest" == e[a].rel && makeAjax(e[a].href);
  },
  testForManifest = void collectManifestObj();
